...
int NumOfMinimalSumRow(int[,] array)
{
    int rowWithMinimalSum = -1;
    ...
    for (int i = 0; i < array.GetLength(0); i++)
    {
        ...
        if (sumOfElements < minSumOfElements)
        {
            minSumOfElements = sumOfElements;
            rowWithMinimalSum = i + 1;
        }
    }
    return rowWithMinimalSum;
}
...
int rowWithMinimalSumOfElem = NumOfMinimalSumRow(array);
Console.WriteLine($"Строка с наименьшей суммой элементов: {rowWithMinimalSumOfElem}");
